...
// ==========================================
// Increased limits for large prescription uploads
app.use(cors({ origin: "*" }));
...
// 🚨 RENDER HEALTH CHECK: Prevents Render from crashing your deployment
app.get('/', (req, res) => {
    res.status(200).send("✅ MediFlow Backend is LIVE and running smoothly!");
});
...
const OrderSchema = new mongoose.Schema({
    orderId: String, 
    userId: String, 
    items: Array, // Fully supports the { name, price, qty } objects from your frontend
    totalAmount: Number,
    addressId: String, 
    status: { type: String, default: "Confirmed" },
    hasPrescription: Boolean, 
    rxImageUrl: String, 
    date: { type: Date, default: Date.now } // Using date to match frontend History sorting
});
...
app.post('/api/auth/send-otp', async (req, res) => {
    ...
    // For testing, we log the OTP. In production, connect Twilio/Msg91 here.
...
app.post('/api/orders', async (req, res) => { 
    try { 
        // Ensures the DB uses the exact Order ID generated by the frontend
...
// 🚨 THIS WAS MISSING: Required for the "My Orders" Tab to work 🚨
app.get('/api/orders/:userId', async (req, res) => {
    try {
        // Fetch order history for the user, sorting by newest first
...
// Update the status of an order (Accept/Reject/Dispatch)
